...
#After doing the Lasso Regression, I noticed that the pelvic tilt varible was always reduced to 0 for all three outcomes
#This allowed me to eliminate it completely from the dataset and remove the colinearrity issue displayed earlier with BIOMECHANICAL.R

Biomech_ <- read.csv("column_3C_weka.csv")
Biomech <- subset(Biomech_, select = -pelvic_tilt)
...
mean(predictions_2$class == test_data_2$class)
